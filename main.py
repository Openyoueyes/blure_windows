...
def print_welcome(language):
    if language == '1':
        print("""To select the Telegram Desktop window, enter 1.""")
    elif language == '2':
        print("""Добро пожаловать в автокликер для Blum. Для выбора окна Telegram Desktop введите 1.""")
                ...
                try:
                    for bx in range(-5, 6):
                        for by in range(-5, 6):
                            ...
                            if bomb_range[0][0] <= br <= bomb_range[1][0] and bomb_range[0][1] <= bg <= bomb_range[1][
                                1] and bomb_range[0][2] <= bb <= bomb_range[1][2]:
                                is_bomb = True
                                break
                        if is_bomb:
                            break
